...
int BleProtocol::DelDev2Room(uint16_t devAddr, uint16_t room, uint16_t scene)
{
	LOGW("Del dev addr: 0x%04X from room: %d, scene: %d", devAddr, room, scene);
	uint8_t dataRsp[100];
	int lenRsp;
...
int BleProtocol::GetInfogw()
{
	LOGD("GetInfogw");
	uint8_t n = 0x02;
	SendMessage(SYSTEM_REQ, &n, 1, 0, 0, 0, 10000);
...
{
	LOGD("RPA_Test_Socket 0x%04x, relay %d, led %d, button %d", devAddr, relay, led, button);
	uint8_t dataRsp[100];
	int lenRsp;
	typedef struct __attribute__((packed))
	{
	...
	cmd.ble_message_header.devAddr = devAddr;
	cmd.opcodeVendor = RD_OPCODE_CONFIG;
	cmd.vendorId = RD_VENDOR_ID;
	cmd.header = RD_HEADER_RPA_TEST_SOCKET;
	cmd.relay = relay;
	cmd.led = led;
	cmd.button = button;
