...
void Udp::UdpOnMessage(string message, struct sockaddr_in *si_other, int slen)
{
	...
	if (payloadJson.parse(message) && payloadJson.isObject())
	{
		...
		else if (payloadJson.isMember("data") &&
             payloadJson["data"].isObject() &&
             payloadJson["data"].isMember("cmd") &&
             payloadJson["data"]["cmd"].isString())
		{
			cmd = payloadJson["data"]["cmd"].asString();
		}
		if (cmd != "")
		{
...
int Udp::send(string message, struct sockaddr_in *si_other, int slen)
{
	...
	{
		LOGW("UDP sendto error, errno=%d (%s)", errno, strerror(errno));
		return CODE_ERROR;
	}
	return CODE_OK;
}

int Udp::SendTo(const std::string &message,
                const std::string &ip,
                uint16_t port)
{
    if (fd < 0)
    {
        LOGE("Socket not ready (fd invalid)");
        return CODE_ERROR;
    }
    ...
    LOGI("Send to %s:%d", ip.c_str(), port);
    ...
    if (ret < 0)
    {
        LOGE("sendTo error, errno=%d (%s)", errno, strerror(errno));
        return CODE_ERROR;
    }
...
int Udp::SendBroadcast(const std::string &message,
                       const std::string &broadcast_ip,
                       uint16_t port)
{
    return SendTo(message, broadcast_ip, port);
}